...
{     
    ...
    $vaules = array(            
     ...
     $url = "http://service.ruitukeji.cn/index.php?m=Home&c=Index&a=user_push&".http_build_query($vaules);
...
//					array('name' => '商品咨询','act'=>'ask_list','control'=>'Comment'),
                    ...
                    array('name' => '线下订单管理','act'=>'complain_list', 'control'=>'Comment'),
			)),
			'Store' => array('name' => '店铺管理', 'icon'=>'fa-home', 'sub_menu' => array(
					...
					array('name' => '经营类目审核', 'act'=>'store_list', 'control'=>'Store'),
			)),
			...
			'promotion' => array('name' => '活动管理', 'icon'=>'fa-bell', 'sub_menu' => array(
					array('name' => '限时秒杀', 'act'=>'flash_sale', 'control'=>'Promotion'),
 ...
 * @param $red_id //红包id
 */
function store_money_pro($red_id){
    $store_list=M('red_store')->where(array("red_id"=>$red_id))->select();
    $red_info['total_money']=0;
    if(count($store_list)>0){
        foreach($store_list as $k=>$v){
            $red_info['total_money']+=$v['contribute_money'];
        }
    }
    //计算店铺中奖概率
    /**
    ...
    foreach ($store_list as $k1=>$v1){
        $money_pro=round(($v1['contribute_money']/$red_info['total_money'])*100,2);
        M('red_store')->where(array("id"=>$v1['id']))->save(array("money_pro"=>$money_pro));
    }
    M('red_package')->where(array("id"=>$red_id))->save(array("total_money"=>$red_info['total_money']));
}

/**
 * 用户提现申请通过 ruit_consume_log 表插入数据
 * ruit_consume_log
 *
 */
function add_consume_log($data){
    //M('consume_log')
}

function userRoleName ($user_id)
{
    $where = "user_id = $user_id AND pay_status = 1 AND status = 1";
    $user_shareholder = getTableInfo("user_shareholder", $where,"*","count");
    if($user_shareholder)
        return 3;
    $user_startup = getTableInfo("user_startup", $where,"*","count");
    if($user_startup)
        return 3;
    $user_entre = getTableInfo("user_entre", $where,"*","count");
    if($user_entre)
        return 2;
    return 1;
}
...
function getTableInfo($table,$where="",$field = "*",$select = "find")
{
    return M($table) -> where($where) -> field($field) -> $select();
}
