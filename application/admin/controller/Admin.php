...
class Admin extends Base {
    ...
    public function adminHandle(){
    	...
    	if($data['act'] == 'edit'){
    	    ...
    	    unset($data['admin_id']);
    		$r = D('admin')->where('admin_id='.$admin_id)->save($data);
    	}
    ...
    public function login(){
        if(IS_POST){
            ...
            if(!empty($condition['user_name']) && !empty($condition['password'])){
                $admin = M('admin')->where(array('user_name' => $condition['user_name']))->find();
                ...
                if(cache('admin_id_'.$admin_info['admin_id'])){
                    exit(json_encode(array('status'=>0,'msg'=>'您的账号已登录,请稍后重试!!')));
                }
                if(is_array($admin_info)){
                    //session(array("name"=>"admin_id","path"=>$admin_info['admin_id'],"expire"=>"60"));
                    session('admin_id',$admin_info['admin_id']);
                    session('act_list',$admin_info['act_list']);
                    session('token',md5(uniqid(microtime(true),true)));
                    M('admin')->where("admin_id = ".$admin_info['admin_id'])->save(array('last_login'=>time(),'last_ip'=>  getIP(),'token'=>session('token')));
                    session('last_login_time',$admin_info['last_login']);
                    session('last_login_ip',$admin_info['last_ip']);
                    cache('admin_id_'.$admin_info['admin_id'],"wkf","1440");
                    adminLog('后台登录',0);
					$this->deleteOldMsg();
    ...
    /*protected function checkAdmin() {
        ...
        $where['userid'] = Session::get('enewsuser_userid');
        $where['rnd'] = Session::get('enewsuser_rnd');
        $enewsuser = Db::name('enewsuser')->field('userid,rnd')->where($where)->find();
        if (!$enewsuser) {
            $this->error('您的账号只能一人在线！', 'admin/login/index');
        }
    }*/
    ...
    public function logout(){
        cache('admin_id_'.session('admin_id'),null);
        session_unset();
        session_destroy();
    ...
    public function role_info()
    {
        ...
        $modules = [];
    	$right = M('system_menu')->where(array('type'=>0,"is_del"=>0))->order('id')->select();
