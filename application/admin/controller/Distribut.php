...
class Distribut extends Base {
    ...
    { 
        ...
        $user_arr = M('users')->where("user_id in (".  implode(',', $user_id_arr).")")->getField("user_id,nickname,email,mobile,level,user_relation",true);
        if(count($user_arr)>0){
            $level_arr=array(1=>"普通会员",2=>"创业者",3=>"合伙人");
            foreach($user_arr as $k=>&$v){
                $v['level_name']=M('user_level')->where(array("level_id"=>$v['level']))->getField('level_name');
                $v['relation_name']=M('user_relation')->where(array("id"=>$v['user_relation']))->getField('level_name');
                //$v['level_str']=$level_arr[$v['level']];
            }
        }
