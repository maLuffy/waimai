...
class Finance extends Base {
    ...
    public function get_withdrawals_list($status=''){
    	...
    	foreach($list as $k=>&$v){
    	    $v['new_money']=$v['money']-$v['employ_money'];
        }
...
//    	}
    	...
    	if($export == 1){
    		...
    		$strTable .= '<tr>';
    		$strTable .= '<td style="text-align:center;font-size:12px;width:120px;">申请人</td>';
    		$strTable .= '<td style="text-align:center;font-size:12px;" width="100">提现金额(已扣除手续费)</td>';
    		$strTable .= '<td style="text-align:center;font-size:12px;" width="*">银行名称</td>';
    		$strTable .= '<td style="text-align:center;font-size:12px;" width="*">银行账号</td>';
    		$strTable .= '<td style="text-align:center;font-size:12px;" width="*">开户人姓名</td>';
    		$strTable .= '<td style="text-align:center;font-size:12px;" width="*">申请时间</td>';
    ...
    public function withdrawals_update(){
        ...
        if($data['status']=="1"){
            $withdrawals_info=M('withdrawals')->where('id in ('.implode(',', $id).')')->select();
            foreach ($withdrawals_info as $k=>$v){
                $user_info=M('users')->where(array("user_id"=>$v['user_id']))->field("user_money,pay_points")->find();
                $add_consume=array();
                $add_consume['user_id']=$v['user_id'];
                $add_consume['type']=0;
                $add_consume['consume_money']=$v['money'];
                $add_consume['create_time']=time();
                $add_consume['order_sn']="";
                $add_consume['desc']="会员提现申请通过";
                $add_consume['user_money']=$user_info['user_money'];
                $add_consume['pay_point']=$user_info['pay_points'];
                $add_consume['status']="1";
                M('consume_log')->add($add_consume);
            }
        }
        if($data['status']=="-1"){
            $withdrawals_info=M('withdrawals')->where('id in ('.implode(',', $id).')')->select();
            foreach ($withdrawals_info as $k=>$v){
                $user_info=M('users')->where(array("user_id"=>$v['user_id']))->setInc("user_money",$v['money']);
                $add_consume=array();
                $add_consume['user_id']=$v['user_id'];
                $add_consume['type']=0;
                $add_consume['consume_money']=$v['money'];
    ...
    public function store_withdrawals_update(){
        ...
        //商家提现如果是拒绝的话  把商家钱退回去
        if($data['status']=="-1"){
            $store_with=M('store_withdrawals')->where('id in ('.implode(',', $id).')')->select();
            foreach($store_with as $k=>$v){
                M('store')->where(array("store_id"=>$v['store_id']))->setInc("store_money",$v['money']+$v['taxfee']);
            }
        }
    	if($r){
    		$this->ajaxReturn(array('status'=>1,'msg'=>"操作成功"),'JSON');
