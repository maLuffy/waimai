...
class Goods extends Base {
    ...
    public function addEditCategory()
    {
        ...
        if (IS_GET)
        {
            ...
            $goods_type_list = M('goods_type')->select(); // 所有类型
    ...
    public function ajaxGoodsList(){ 
		...
		foreach($goodsList as $k=>&$v){
            $store_is_own_shop=M('store')->where(array("store_id"=>$v['store_id']))->field("store_name,is_own_shop")->find();
            $v['is_own_shop']=$store_is_own_shop['is_own_shop'];
            $v['store_name']=$store_is_own_shop['store_name'];
        }
		//$this->assign('store_list',$store_list);
        $catList = M('goods_category')->cache(true)->select();
        $catList = convert_arr_key($catList, 'id');
        $store_type = array('加盟店','自营');
     ...
     *  商品列表
     */
    public function goodsList_copy(){
        $GoodsLogic = new GoodsLogic();
        $brandList = $GoodsLogic->getSortBrands();
        $categoryList = $GoodsLogic->getSortCategory();
        $this->assign('categoryList',$categoryList);
     ...
     *  商品列表
     */
    public function ajaxGoodsList_copy(){
    ...
    public function act()
    {
        ...
        if ($act == 'is_find_good') {
            $new_condition['goods_id'] = array('in', $goods_ids);
            M('goods')->where($new_condition)->save(array('is_find_good' => 1));
            $this->ajaxReturn($return_success);
        }
        //优品推荐
        if ($act == 'is_youpin') {
            $new_condition['goods_id'] = array('in', $goods_ids);
            M('goods')->where($new_condition)->save(array('is_youpin' => 1));
            $this->ajaxReturn($return_success);
        }
        //是否排行榜
        if ($act == 'is_rank') {
            $new_condition['goods_id'] = array('in', $goods_ids);
            M('goods')->where($new_condition)->save(array('is_rank' => 1));
            $this->ajaxReturn($return_success);
        }
        //是否家具
        if ($act == 'is_nice') {
            $new_condition['goods_id'] = array('in', $goods_ids);
            M('goods')->where($new_condition)->save(array('is_nice' => 1));
            $this->ajaxReturn($return_success);
        }
        if($act =='takeoff'){
