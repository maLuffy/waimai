...
//    	$today = strtotime("-1 day");
    ...
    public function changeTableVal(){  
            ...
            $value  = I('value'); // 修改字段值
            if($table=="goods_category" && $field=='is_hot'){
                M($table)->where("$id_name = $id_value")->save(array($field=>$value));
                $map=array();
                $category=M($table)->where(array("id"=>$id_value))->find();
                $map['level']=array("gt",$category['level']);
                $map['parent_id_path']=array("like","%$id_value%");
                $list=M($table)->where($map)->select();
                $cat_id=array();
                foreach($list as $k=>$v){
                    M($table)->where(array("id"=>$v['id']))->save(array($field=>$value));
                    $cat_id[]=$v['id'];
                }
                return $this->ajaxReturn($cat_id);
            }
