...
//    	$log =  M('order_action'.$select_year);
    	...
    	$admin_id = I('admin_id');
		if($admin_id){
		    $condition['oa.user_type']="2";
			$condition['u.nickname'] = array("like","%$admin_id%");
		}
    	$count = $OrderActionModel->alias('oa')->join('users u','oa.action_user = u.user_id','left')->where($condition)->count();
    	$Page = new Page($count,20);    	 
    	$show = $Page->show();
    	$list = $OrderActionModel->where($condition)->alias('oa')
            ->field('oa.*,u.user_id,u.nickname,s.store_name')
            ->join('users u','oa.action_user = u.user_id and u.user_id=2','left')
            ->join('store s','oa.store_id = s.store_id and s.store_id=1','left')
