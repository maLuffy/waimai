...
//                    M('shipping_area')->add($add);
    ...
    public function setting(){
        ...
        if(($condition['code'] == 'unionpay') || ($condition['type'] == 'login'  && $condition['code'] == 'weixin'))
        {
    ...
    public function shipping_print(){
        ...
        if(empty($shipping['config']) || unserialize($shipping['config']) == null){
    ...
    public function add_shipping()
    {
        ...
        $name = I('name'); // 物流名字
        $desc = I('desc', '');// 插件描述
        $baseDir = "/home/wwwroot/huada/huada_seller/";
        $adminBaseDir = "/home/wwwroot/huada/huada_admin/";
        $dir = $baseDir."/plugins/shipping/" . $code;
        $adminDir = $adminBaseDir."/plugins/shipping/" . $code;
        ...
        if ($shipping) {
            $this->error("编码" . $code . "已存在");
        }
        $shipping = Db::name('plugin')->where('type','shipping')->where('name',$name)->find();
        if($shipping){
            $this->error("物流公司名字" . $name . "已存在");
        }
        if (!file_exists($dir)) {
            mkdir($dir);
        }
        ...
        if ($_FILES['shipping_img']['tmp_name']) {
            ...
            $info = $upload->validate(['size' => $image_upload_limit_size, 'ext' => 'jpg,png,gif,jpeg'])->move($dir . '/', 'logo.jpg');
            if ($info) {
                $config_html = "<?php
                        return array(
                            'code'=> '$code',
                ...
                file_put_contents($baseDir."/plugins/shipping/$code/config.php", $config_html);
                $logoDir =  '/logo.jpg';
                $phpDir =  '/config.php';
                copy($dir .$logoDir ,$adminDir.$logoDir);
                copy($dir .$phpDir ,$adminDir.$phpDir);
    ...
    public function del_shipping($code){
        ...
        {
            delFile($baseDir.$dir); // 删除 物流配置
            rmdir($baseDir.$dir); // 删除 物流配置
        }
        if(file_exists($adminBaseDir.$dir))
        {
            delFile($adminBaseDir.$dir); // 删除 物流配置
            rmdir($adminBaseDir.$dir); // 删除 物流配置
        }
