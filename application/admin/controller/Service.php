...
class Service extends Base{
	...
	public function return_list(){
        ...
        $begin && $this->assign('start_time', date("Y-m-d H:i:s",$begin));
        $end && $this->assign('end_time', date("Y-m-d H:i:s",$end));
	...
	public function refund_list(){
        ...
        $begin && $this->assign('start_time', date("Y-m-d",$begin));
        $end && $this->assign('end_time', date("Y-m-d",$end));
	...
	public function refund_info(){
		if(IS_POST){
            ...
            $CommonOrderLogic->alterReturnGoodsInventory($final_return_info,$final_return_info['rec_id']);//取消订单后改变库存
	...
	public function expose_subject_info(){
		if(IS_POST){
			...
			if(M('expose_subject')->add($data)){
                $this->ajaxReturn(['status'=>0,'msg'=>'添加成功','url'=>U('Service/expose_subject_list')]);
			}else{
                $this->ajaxReturn(['status'=>0,'msg'=>'添加失败,']);
			}
		}
