...
class ShopOrder extends Base {
    ...
    public function ajaxindex(){
        ...
        if($nickname)
        {
            $user_id_arr = M('users')->where("nickname like '%$nickname%'")->getField('user_id',true);
            if($user_id_arr)
            {
                $condition['u.user_id'] = array('in',$user_id_arr);
            }
        }
        I('pay_status') != '' ? $condition['pay_status'] = I('pay_status') : false;
        I('order_statis_id') != '' ? $condition['order_statis_id'] = I('order_statis_id') : false; // 结算统计的订单
        $count = M('shop_pay s')->join('users u','u.user_id = s.user_id','LEFT')->where($condition)->count();
        $Page  = new AjaxPage($count,20);
        $show = $Page->show();
        //获取订单列表
        $orderList = M('shop_pay s')->join('users u','u.user_id = s.user_id','LEFT')->where($condition)->order("id desc")->limit("{$Page->firstRow},{$Page->listRows}")->select();
        foreach($orderList as $k=>&$v){
            $v['shop_info'] = M('shop')->where(array("shop_id"=>$v['shop_id']))->find();
        }
    ...
    */
    ...
    public function detail($order_sn){
        if($order_sn){
            $shop_pay_info=M('shop_pay s ')->join('users u','u.user_id = s.user_id','LEFT')->field('s.*,u.*')->where(array("order_sn"=>$order_sn))->find();
            $shop_pay_info['shop']=M('shop')->where(array("shop_id"=>$shop_pay_info['shop_id']))->find();

            //获取价格调整了多少钱
            $shop_pay_info['order_prom_amount']=$shop_pay_info['integral_money']+$shop_pay_info['coupon_price'];
            //获取下单人地址
            $province = M('region')->where(array('id' => $shop_pay_info['province']))->find();
            $city = M('region')->where(array('id' => $shop_pay_info['city']))->find();
            $area = M('region')->where(array('id' => $shop_pay_info['district']))->find();

            $province_shop = M('region')->where(array('id' => $shop_pay_info['shop']['province']))->find();
            $city_shop = M('region')->where(array('id' => $shop_pay_info['shop']['city']))->find();
            $area_shop = M('region')->where(array('id' => $shop_pay_info['shop']['district']))->find();
            $this->assign('address2',$province['name'].$city['name'].$area['name']);
            $this->assign('address',$province_shop['name'].$city_shop['name'].$area_shop['name']);
            $this->assign('order',$shop_pay_info);
            return $this->fetch();
        }else{
            $this->error("请输入订单号");
        }
...
// 虚拟订单列表
    ...
    public function edit_order(){
       	...
       	if(IS_POST)
        {
            ...
            $result = calculate_price($order['user_id'],$goodsArr,$order['shipping_code'],$order['province'],$order['city'],$order['district'],0,0,0,0);
            if($result['status'] < 0)
            {
    ...
    public function split_order(){
    	...
    	if(IS_POST){
    		...
    		$result = calculate_price($order['user_id'],$oldArr,$order['shipping_code'],$order['province'],$order['city'],$order['district'],0,0,0,0);
    		if($result['status'] < 0)
    		{
    			$this->error($result['msg']);
    		}
    		...
    		foreach($brr as $goods){
    			$result = calculate_price($order['user_id'],$goods,$order['shipping_code'],$order['province'],$order['city'],$order['district'],0,0,0,0);
    			if($result['status'] < 0)
    			{
    				$this->error($result['msg']);
    			}
...
//    	$log =  M('order_action'.$select_year);
    ...
    public function add_order()
    {
        ...
        if(IS_POST)
        {
            ...
            $result = calculate_price($order['user_id'],$order_goods,$order['shipping_code'],$order[province],$order[city],$order[district],0,0,0,0);
            if($result['status'] < 0)	
            {
                 $this->error($result['msg']);      
            } 
