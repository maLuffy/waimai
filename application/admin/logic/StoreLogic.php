...
{    
    ...
    public function addStore($store)
    {
		...
		//添加店铺管理员
		$seller = array('seller_name'=>$store['seller_name'],'store_id'=>$store_id,'user_id'=>$store['user_id'],'is_admin'=>1,'add_time'=>time());
    ...
    public function auto_transfer($store_id){
        ...
        M('order')->where("order_id in (".  implode(',', $order_id_arr).")")->save(array('order_statis_id' =>$order_statis_id,'check_time' => time())); // 标识为已经结算
        // 给商家加钱 记录日志
