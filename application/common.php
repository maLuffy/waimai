...
//    $original_img = '.' . $original_img; // 相对路径
    $original_img = $_SERVER["DOCUMENT_ROOT"] . $original_img; // 相对路径
...
function accountLog($user_id, $user_money = 0, $pay_points = 0,$desc = '', $distribut_money = 0, $order_id = 0 ,$order_sn = '',$frozen_money=0)
{
    ...
    //获取账户原积分和可用余额
    $user_info=M('users')->where(array("user_id"=>$user_id))->field("user_money,pay_points")->find();

    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id' => $user_id,
        'current_money' => $user_money+$user_info['user_money'],
        'current_points'=>$pay_points+$user_info['pay_points'],
 ...
 * @param $parent_id
 */
function getNextArea ($parent_id)
{
    return M("region") -> where("parent_id = $parent_id") -> select();
}

function insertStoreInfo ($user_id, $store_data, $userData)
{
    if($store_data)
    {
        db::name("user_store") -> where("user_id = $user_id") -> delete();
        foreach($store_data as $key => &$val)
        {
            if(empty($val["store_name"]))
                continue;

            if((!empty($val["province"]) || !empty($val["store_detail_addr"])) && empty($val["store_name"]))
                ajaxReturn(['status' => 0,'msg'=>'店铺名称不能为空！']);
            $val["user_id"] = $user_id;
            $val["province"] = intval($val["province"],0);
            $val["city"] = intval($val["city"],0);
            $val["district"] = intval($val["district"],0);
            $val["mobile"] = $userData["mobile"];
            $val["store_time"] = time();
            $result[] = $val;
        }
        if($result)
            return db::name("user_store") -> insertAll($result);
    }
}

function getGooodsThumb ($goods_id)
{
    if($goods_id)
    {
        $orginal_img = M("goods") -> where("goods_id = ".$goods_id) -> value("original_img");
        if($orginal_img)
            return $orginal_img;
    }
    return "/public/images/icon_goods_thumb_empty_300.png";
...
function template_add($a,$b){
    echo sprintf("%1\$.2f",intval($a)+intval($b));
}
/**
 * 相减，供模板使用
 * @param <type> $a
