...
class IntegralLogic extends Model
{
    ...
    public function order()
    {
        ...
        $store_free_price = Db::name('store')->where("store_id", $this->goods['store_id'])->cache(true, 100)->getField('store_free_price');
        if (empty($this->specGoodsPrice)) {
            //没有规格
        ...
        if ($this->goods['is_free_shipping'] == 0 && ($store_free_price == 0 || $this->structure['goods_price'] < $store_free_price)){
            $GoodsLogic = new GoodsLogic();
            $this->structure['shipping_price'] = $GoodsLogic->getFreight($this->shippingCode, $this->address['province'], $this->address['city'], $this->address['district'],$this->goods['weight'] * $this->buyNum , $this->goods['store_id']);
        }
    ...
    public function addOrder($invoice_title,$user_note){
        ...
        $orderGoodsData = [
            ...
            'store_id'  => $this->goods['store_id'],// 店铺id
            'distribut' => $this->goods['distribut'],// 三级分销金额
            'commission' => Db::name('goods_category')->where("id", $this->goods['cat_id3'])->cache(true,TPSHOP_CACHE_TIME)->getField('commission'), // 商品抽成比例
        ];
        if(empty($this->specGoodsPrice)){
        ...
        if($res && $res['status'] ==1){
            $store = Db::name('store')->where("store_id", $this->goods['store_id'])->find();
