...
class TeamOrderLogic extends Model
{
    ...
    public function useShipping($shipping_code, $UserAddress)
    {
        ...
        if ($shipping_code) {
            ...
            if ($shipping) {
                ...
                if ($this->goods['is_free_shipping'] == 0 && ($this->store['store_free_price'] == 0 || $this->order['goods_price'] < $this->store['store_free_price'])) {
    ...
    public function pushSellerMsg(){
        if(!empty($this->store)){
            ...
            if($res && $res['status'] == 1){
                $sender = (!empty($this->store['service_phone'])) ? $this->store['service_phone'] : false;
