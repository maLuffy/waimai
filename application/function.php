...
function orderStatus ($orderInfo)
{
    if($orderInfo["pay_status"] == 0)
        return "未支付";
    if($orderInfo["pay_status"] == 1 AND $orderInfo["shipping_status"] == 0 )
        return  "未发货";
    if($orderInfo["pay_status"] == 1 AND $orderInfo["shipping_status"] == 1 && $orderInfo["order_staus"] == 1 )
        return  "已确认";
    if($orderInfo["pay_status"] == 1 AND $orderInfo["shipping_status"] == 1 && $orderInfo["order_staus"] == 2 )
        return "已确认";
    if($orderInfo["pay_status"] == 1 AND $orderInfo["shipping_status"] == 1 && $orderInfo["order_staus"] == 3 )
        return "已取消";
    if($orderInfo["pay_status"] == 1 AND $orderInfo["shipping_status"] == 1 && $orderInfo["order_staus"] == 4 )
        return "已完成";
    if($orderInfo["pay_status"] == 1 AND $orderInfo["shipping_status"] == 1 && $orderInfo["order_staus"] == 5 )
        return "已作废";
}

function selfCacheGet($name)
{
    $options =
        [
            'type'  =>  'File',
            ...
            'path'  =>  '/home/wwwroot/huada/huada_api/runtime/cache/',
        ];
...
function selfCacheWrite($name,$value,$expire=0)
{
    $cache = new \think\Cache;
    $options =
        [
