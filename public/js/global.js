 ...
 * @param t  省份select对象
 */
function get_city(t,selected,id)
{
    var parent_id = $(t).val();
    if(!parent_id > 0){
        return;
    ...
    var url = '/index.php?m=Admin&c=Api&a=getRegion&level=2&parent_id='+ parent_id+"&selected="+selected;
    $.ajax({
        type : "GET",
        url  : url,
        error: function(request) {
        ...
        success: function(v) {
            ...
            $('#district'+id).empty().html('<option value="0">选择区域</option>');
        }
    });
}
...
function GetUploadify3(num,elementid,path,callback){	   	
	common_uploadify(num,elementid,path,callback , 'Seller');
}
...
function get_category(id,next,select_id){
    var url = '/index.php?m=Admin&c=api&a=get_category&parent_id='+ id;
    $.ajax({
        type : "GET",
        url  : url,
        error: function(request) {
