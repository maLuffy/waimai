...
class Page{
    ...
    private $config  = array(
        ...
        */
        ...
        'first'  => '首页',
        'last'   => '尾页',
        //'sel'    => '<input class="rows" type="text">',
        'theme'  => '%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %SEL%',
    );
...
//                    $link_page .= '<span class="current">' . $page . '</span>';
        ...
        //替换分页内容
        $this->config['theme']='%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%';
    ...
    public function show_page() {
        ...
        $this->config['header'] = $this->totalRows > 0 ? '<li id="example1_previous" class="paginate_button"><a class="prev">共'. $this->totalPages .'页</a></li>' : '';
        ...
        //输入页码查询
        /* $the_sel = '';*/
        /*$the_sel = '<div id="example1_previous" class="paginate_button previous disabled"><input style="margin-right: 30px" type="text" id="sel_p" name="p" value="" placeholder="输入页码"><a>跳转</a></div>';
        */


        $the_sel='<form action="'.$this->url($this->nowPage).'" method="get"><div id="example1_previous" class="paginate_button previous disabled"><input style="margin-right: 10px;width: 60px;" type="text" id="sel_p" name="p" value="'.$this->nowPage.'" placeholder="输入页码"><button class="btn_q" type="sumbit">跳转</button></div></form>';
...
//                    $link_page .= '<span class="current">' . $page . '</span>';
        ...
        //替换分页内容
        $page_str = str_replace(
            array('%HEADER%', '%NOW_PAGE%', '%UP_PAGE%', '%DOWN_PAGE%', '%FIRST%', '%LINK_PAGE%', '%END%','%SEL%', '%TOTAL_ROW%', '%TOTAL_PAGE%'),
            array($this->config['header'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end,$the_sel, $this->totalRows, $this->totalPages),
            $this->config['theme']);
        return "<div class='dataTables_paginate paging_simple_numbers'><ul class='pagination'>{$page_str}</ul></div>";
    }
